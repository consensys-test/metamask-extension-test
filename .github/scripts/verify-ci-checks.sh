...
# TEST_MODE bypass for fork testing without real CI runs
if [[ "${TEST_MODE:-}" == "true" ]]; then
    echo "=============================================="
    echo "⚠️  TEST_MODE enabled - bypassing CI check verification"
    echo "=============================================="
    echo ""
    echo "In production, this script verifies that 'all-jobs-pass'"
    echo "CI check has completed successfully on RELEASE_SHA."
    echo ""
    echo "SHA that would be verified: ${RELEASE_SHA:-not set}"
    echo ""
    echo "✅ CI check verification SKIPPED (test mode)"
    exit 0
fi
